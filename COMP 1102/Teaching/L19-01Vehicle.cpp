...
Car::Car(string id, int dist, int num_seats)
{
	this->id=id;
	dist_travelled=dist;
...
{
	cout << "vehicle id: " << id << " dist travelled: " << dist_travelled <<  
	     " num seats: " << num_seats << endl;
}

Car::~Car()
{
  cout << "destroyed the car";
}
...
Motorcycle::Motorcycle(string id, int dist, bool sidecar)
{
	this->id=id;
	dist_travelled=dist;
...
int main()
{
  ...
  Vehicle *myVehicle=new Vehicle("123BRAD",30000);
  Car *myCar=new Car("456BRAD",70000,4);
  Motorcycle *myCycle=new Motorcycle("789SUSIE",15000,true);
